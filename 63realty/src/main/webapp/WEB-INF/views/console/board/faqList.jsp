	...
	<div class="search_box">
		...
		<select class="form_select" name="codeSeq">			
			...
			<c:forEach var="comm" items="${commList}" varStatus="status">
				<option value="${comm.codeSeq}" ${searchVO.codeSeq eq comm.codeSeq?'selected':''}>${comm.codeName}</option>
			</c:forEach>
		</select>
			...
			<tbody>
				...
				<c:forEach items="${list}" var="vo" varStatus="status" >
					<tr>
						...
						<td>${vo.codeName}</td>
						<td><a href="/console/board/faq_modify/${vo.mainBoardSeq}">${vo.title}</a></td>
