...
public class ExcelViewReader extends AbstractXlsxView {
	...
	protected void buildExcelDocument(Map<String, Object> model, Workbook workbook, HttpServletRequest request,
		...
		List<ReReaderVO> colValue = (List<ReReaderVO>) model.get("list");

		response.setContentType("application/msexcel");
		response.setHeader("Content-Disposition", "attachment; filename="
				+ new String(excelName.getBytes("MS949"), "8859_1") + ".xls");
		...
		sheet.setColumnWidth(6, (short)8000);
		...
		for(ReReaderVO v : colValue){
			Row row = sheet.createRow(i + k);
			Cell cell0 = row.createCell(0);
			cell0.setCellValue("Y".equals(v.getUserFlag())?"구독하기":"구독취소");
			cell0.setCellStyle(cs);
			
			Cell cell1 = row.createCell(1);
			cell1.setCellValue(v.getReaderName());
			cell1.setCellStyle(cs);
			
			String []phone = commonUtil.divideMobile(v.getReaderPhone());
			String phoneNum="";
			for(String s : phone) {
				phoneNum=phoneNum+s+"-";
			}
			Cell cell2 = row.createCell(2);
			cell2.setCellValue(phoneNum.substring(0,phoneNum.length()-1));
			cell2.setCellStyle(cs);
			
			Cell cell3 = row.createCell(3);
			cell3.setCellValue(v.getEmail());
			cell3.setCellStyle(cs);
			
			Cell cell4 = row.createCell(4);
			cell4.setCellValue(commonUtil.chgDateFormat("yyyy-MM-dd HH:mm:ss",v.getRegDate()));
			cell4.setCellStyle(cs);
			
			Cell cell5 = row.createCell(5);
			cell5.setCellValue(commonUtil.chgDateFormat("yyyy-MM-dd HH:mm:ss",v.getCancelDate()));
			cell5.setCellStyle(cs);
			
			Cell cell6 = row.createCell(6);
			cell6.setCellValue(v.getCancelName());
